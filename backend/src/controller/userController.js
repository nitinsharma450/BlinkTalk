...
static async getAllUser(req,res){
    try {
        ...
        const users=await UserSchema.find({en:{loggedInUser}}).select('userName about profilePicture isOnline phoneNo PhoneSuffix lastSeen');

        const usersWithConversation =Promise.all(users.map((user)=>{
        ...
        res.status(200).json({message:"Users found successfully", response:usersWithConversation})
    } catch (error) {
        res.status(500).json({message:"Internal server error"})
    }
}
